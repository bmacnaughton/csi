...
// name: present without the prefix if not environmentalized version of key.
...
async function get (options = {}) {
  ...
  // function to aggregate array-based return values. all keys other than the
  // values are informational (info) keys.
  function copyInfo (info) {
    if (typeof info !== 'object') return;
    // if this info has a key matching a results key concatenate it.
    infoKeys.forEach(k => {
      if (info[k]) {
...
async function convert (key, value, type) {
  ...
  if (type === 'b') {
    if (typeof value === 'string') {
      // for booleans specifying an empty string (env var) is true. only if the value is
      // explicitly NOT one of the below, e.g., "false", is it false.
